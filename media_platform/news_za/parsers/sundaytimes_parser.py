# -*- coding: utf-8 -*-
"""
Sunday Times (sundaytimes.timeslive.co.za) 解析器
Arena Holdings / Arc Publishing (Fusion engine)，与 Sowetan 同一平台
"""
...
class SundayTimesParser(BaseNewsParser):
    site_key = "sundaytimes"
    site_name = "Sunday Times"
    ...
    def parse_article_list(self, html: str) -> List[str]:
        ...
        for a in soup.select(
            ...
            if url and url not in urls and ("/news/" in url or "/politics/" in url or "/opinion/" in url):
                urls.append(url)
        if not urls:
            for a in soup.select("a[href]"):
                href = a.get("href", "")
                url = self._abs_url(href)
                if url and url not in urls and "timeslive.co.za" in url and len(href) > 30:
                    urls.append(url)
        return urls
